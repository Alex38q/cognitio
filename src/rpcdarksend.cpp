...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <cognitioaddress> <amount>\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in cognitio.conf\n"
                ...
                "  vote         - Vote on a Cognitio initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CCognitioSecret(secret).ToString();
    }
    ...
    if(strCommand == "status")
    {
        ...
        if (pmn) mnObj.push_back(Pair("pubkey", CCognitioAddress(pmn->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Cognitio initiative (can be additionally filtered, partial match)\n"

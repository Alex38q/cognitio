...
const string strMessageMagic = "Cognitio Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 3 * COIN; // instamine prevention
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 > 500 && pindexBest->nHeight+1 <= 210000)  {
        nSubsidy = 5 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 500) {
	ret = 0;
    }else if (nHeight >= 500 && nHeight <= 210000){
	ret = blockValue *4/5;
    } else if (nHeight >= 210000 && nHeight <= 310000) {
        ret = blockValue * 25 / 35; 

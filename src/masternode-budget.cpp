...
int GetBudgetPaymentCycleBlocks()
{
    ...
    if (Params().NetworkID() == Params().Network::MAIN) return 43200;
...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            if (o.scriptPubKey == findScript) {
                LogPrint("mnbudget", "Final Budget: o.nValue(%ld) >= BUDGET_FEE_TX(%ld) ?\n", o.nValue, GetBudgetSystemCollateralAmount(nBestHeight));
                if(o.nValue >= GetBudgetSystemCollateralAmount(nBestHeight)) {
                    foundOpReturn = true;
                }
            }
        }
    ...
    */
    ...
    if (nBlockHash != uint256(0)) {
        BlockMap::iterator mi = mapBlockIndexH.find(nBlockHash);
        if (mi != mapBlockIndexH.end() && (*mi).second) {
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (Params().NetworkID() == Params().Network::TESTNET) {
    ...
    if (!GetTransaction(txidCollateral, txCollateral, nBlockHash)) {

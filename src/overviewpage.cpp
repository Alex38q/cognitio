...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCognitioAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCognitioAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCognitioAmount = strAnonymizeCognitioAmount.remove(strAnonymizeCognitioAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCognitioAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCognitioAmount*COIN) nMaxToAnonymize = nAnonymizeCognitioAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCognitioAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCognitioAmount));
        strAnonymizeCognitioAmount = strAnonymizeCognitioAmount.remove(strAnonymizeCognitioAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCognitioAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
        ...
        strAmountAndRounds = "<span style='color:red;'>" +
                QString(BitcoinUnits::factor(nDisplayUnit) == 1 ? "" : "~") +
